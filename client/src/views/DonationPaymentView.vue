...
// Sample donation campaigns (same as in DonationView) - Translate titles
const campaigns = ref([
  {
    id: 1,
...
const processPayment = () => {
  ...
  setTimeout(() => {
    router.push(
      `/donation/${campaignId}/success?amount=${donationAmount}&currency=${donationCurrency.value}`,
    )
  }, 1500)
}
</script>
...
<template>
  <div class="flex flex-col min-h-dvh bg-gray-100">
    ...
    <div v-if="isCrypto" class="bg-white p-4 mb-4">
      <h3 class="font-medium mb-3">Sambungkan Dompet Anda</h3>
    ...
    <div class="bg-white p-4 mb-4">
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center space-x-2">
          <input type="checkbox" id="anonymous" v-model="isAnonymous" class="h-4 w-4" />
          <label for="anonymous" class="text-sm">Jadikan derma saya tanpa nama</label>
        </div>
      </div>
    </div>
    ...
    <div class="p-4 bg-white sticky bottom-0 border-t">
      ...
      >
        ...
        <span v-else-if="isCrypto && !walletConnected">Sambungkan Dompet untuk Teruskan</span>
        <span v-else>Bayar {{ formatAmount(donationAmount) }}</span>
      </Button>
      <p class="text-xs text-center text-gray-500 mt-2">
        Dengan menderma, anda bersetuju dengan Terma Perkhidmatan dan Dasar Privasi kami
      </p>
    </div>
  </div>
</template>
